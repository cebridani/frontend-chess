...
async function getEvaluation() {
  ...
  try {
    const response = await axios.post('http://34.175.53.49:80/api/chess/get_evaluation', fen, {
      headers: {
...
async function clearMoves() {
  ...
  try {
    const response = await axios.post('http://34.175.53.49:80/api/chess/set_fen', {after}, {
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('access_token')}`
      }
...
async function getBestMove(jwtToken) {
  try {
    const response = await axios.post('http://34.175.53.49:80/api/chess/get_best_move', lastMove, {
      headers: {
        'Content-Type': 'application/json',
...
async function fetchPgn() {
  ...
  try {
    ...
    const response = await axios.get('http://34.175.53.49:80/api/chess/get_pgn', {
      headers: {
...
async function get_list_best_moves() {
  try {
    ...
    const token = localStorage.getItem('access_token');
    const response = await axios.post('http://34.175.53.49:80/api/chess/top_moves', fen, {
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
      }
...
async function saveFen() {
  try {
    const response = await axios.post('http://34.175.53.49:80/api/chess/set_fen', lastMove);
...
async function savePgn() {
  try {
    const response = await axios.post('http://34.175.53.49:80/api/chess/set_pgn', boardAPI.value?.getPgn());
